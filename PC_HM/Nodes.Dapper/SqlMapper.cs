...
namespace Nodes.Dapper
{
    ...
    public class ColumnNameAttribute : Attribute, SqlMapper.INamedMemberItem
    {
        private string name;
    ...
    public static partial class SqlMapper
    {
        ...
        private static IEnumerable<PropertyInfo> FilterParameters(IEnumerable<PropertyInfo> parameters, string sql)
        {
            ...
            //如果参数中包含unicode字符，请使用下面的方法
            //https://code.google.com/p/dapper-dot-net/issues/attachmentText?id=125&aid=1250000000&name=SqlMapper.cs.patch&token=bG_XWnu9Ms47GGRKJCSLh3r5iy4%3A1362645015012
            //return parameters.Where(p => Regex.IsMatch(sql, "[@:]" + p.Name + "\\b", RegexOptions.IgnoreCase | RegexOptions.Multiline));
        }
...
#pragma warning restore 618
        ...
        static List<PropFieldInfo> GetSettableProps(Type t)
        {
            return t
                    ...
                    .Select(p => new PropFieldInfo
                    {
                        ...
                        Type = p.PropertyType,
                        SqlName = ((INamedMemberItem)(p.GetCustomAttributes(typeof(INamedMemberItem), true).FirstOrDefault()) ?? new ColumnNameAttribute(p.Name)).GetName(p)
                    })
        ...
        static List<PropFieldInfo> GetSettableFields(Type t)
        {
            return t
                    ...
                    .Select(f => new PropFieldInfo
                    {
                        ...
                        Field = f,
                        Type = f.FieldType,
                        SqlName = ((INamedMemberItem)(f.GetCustomAttributes(typeof(INamedMemberItem), true).FirstOrDefault()) ?? new ColumnNameAttribute(f.Name)).GetName(f)
                    })
                    .ToList();
        }
    ...
    public class DynamicParameters : SqlMapper.IDynamicParameters
    {
        ...
        public void AddOut(string name, DbType dbType)
        {
            Add(name, null, dbType, ParameterDirection.Output, null);
        }

        public void AddOut(string name, DbType dbType, int? size)
        {
            Add(name, null, dbType, ParameterDirection.Output, size);
        }
