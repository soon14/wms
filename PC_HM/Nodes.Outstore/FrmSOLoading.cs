...
{
    public partial class FrmSOLoading : DevExpress.XtraEditors.XtraForm
    {
        ...
        public string GetBillNOS(int vhID)
        {
            ...
            try
            {
                #region 请求数据
                System.Text.StringBuilder loStr = new System.Text.StringBuilder();
                loStr.Append("vhId=").Append(vhID);
                string jsonQuery = WebWork.SendRequest(loStr.ToString(), WebWork.URL_GetBillNOS);
                if (string.IsNullOrEmpty(jsonQuery))
                {
                    MsgBox.Warn(WebWork.RESULT_NULL);
                    //LogHelper.InfoLog(WebWork.RESULT_NULL);
                ...
                JsonGetBillNOS bill = JsonConvert.DeserializeObject<JsonGetBillNOS>(jsonQuery);
                if (bill == null)
                {
                    MsgBox.Warn(WebWork.JSON_DATA_NULL);
                    return ret;
