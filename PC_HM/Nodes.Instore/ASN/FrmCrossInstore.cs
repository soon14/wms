...
namespace Nodes.Instore
{
    public partial class FrmCrossInstore : DevExpress.XtraEditors.XtraForm
    {
            ...
            try
            {
                ...
                foreach (JsonQueryOverStockBillsResult jbr in bill.result)
                {
                    ...
                    catch (Exception msg)
                    {
                        ...
                        //LogHelper.errorLog("FrmCrossInstore+QueryOverStockBills", msg);
                    }
                    #endregion
        ...
        private void ReloadAsn()
        {
            try
            {
                //List<AsnBodyEntity> result = QueryOverStockBills(GlobeSettings.LoginedUser.WarehouseCode, "20");
                bindingSource1.DataSource = QueryOverStockBills(GlobeSettings.LoginedUser.WarehouseCode, "20");
        ...
        /// 更新订单明细的收货量
        /// </summary>
        public bool SaveOverStock(PODetailEntity entity,string username)
        {
            try
            {
                #region 请求数据
                ...
                //loStr.Append("id=").Append(entity.DetailID);
                loStr.Append("entity=").Append(JsonConvert.SerializeObject(entity)).Append("&");
                loStr.Append("userName=").Append(username);
                string jsonQuery = WebWork.SendRequest(loStr.ToString(), WebWork.URL_SaveOverStock);
                if (string.IsNullOrEmpty(jsonQuery))
                {
                    MsgBox.Warn(WebWork.RESULT_NULL);
                    return false;
